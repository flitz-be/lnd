...
LNDINIT_TAGS := kvdb_etcd kvdb_postgres
...
build:
	...
	cd cmd/lndinit; $(GOBUILD) -tags="$(LNDINIT_TAGS)" -o ../../lndinit-debug $(DEV_LDFLAGS) .
...
install:
	...
	cd cmd/lndinit; $(GOINSTALL) -tags="$(LNDINIT_TAGS)" $(LDFLAGS) .
...
release-install:
	...
	cd cmd/lndinit; env CGO_ENABLED=0 $(GOINSTALL) -v -trimpath -ldflags="$(RELEASE_LDFLAGS)" -tags="$(LNDINIT_TAGS)" .
...
release: clean-mobile
	...
	$(VERSION_CHECK)
	./scripts/release.sh build-release "$(VERSION_TAG)" "$(BUILD_SYSTEM)" "$(RELEASE_TAGS)" "$(RELEASE_LDFLAGS)" "$(LNDINIT_TAGS)"
